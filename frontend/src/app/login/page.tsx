  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 p-6 dark:bg-zinc-950">
      <div className="w-full max-w-md rounded-3xl border border-zinc-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <p className="mb-6 text-sm text-zinc-500">Sign in securely to manage all connected platforms.</p>
        <form className="space-y-3" onSubmit={onSubmit}>
          ...
          <button type="submit" disabled={loading} aria-label="Continue" className="w-full rounded-xl bg-zinc-900 py-2 text-white disabled:opacity-60 dark:bg-zinc-100 dark:text-zinc-900">{loading ? "Signing in..." : "Continue"}</button>
        </form>
      </div>
    </div>
  );
